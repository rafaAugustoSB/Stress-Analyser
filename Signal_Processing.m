...
for m = 1 : 1
    ...
    nomDuFichierCSV = ['C:\Users\Rafael\Desktop\EchoGreeN\MATLAB\Experiments\' nom '.csv'];
...
%     title('Analyse spectral de la donnée brute')
%     xlabel('Fréquence (Hz)')
%     ylabel('Magnitude (dB)')
%     grid on
%     subplot(2, 1, 2)
...
%      save([nom '_signal_filtre_lowPass'], 'dataLowFiltre');
    ...
    legend('envelope des données', ['debut region stress = ' num2str(tDebutStress) 's' ...
    ...
    legend('Derivé de l''envelope des données de la région de stress');
    xlabel('Temps (s)')
    ylabel('Amplitude')
    grid on
    ...
    xlabel('Temps (s)')
    ylabel('Amplitude (V)')
    legend(['Signal filtré low-pass à ' num2str(fBanPassLimHigh) ' Hz'], ...
        ['Signal filtré band-pass [' num2str(fBanPassLimLow) ' ' ...
